...
export class GetRecommendedBooksService {
    ...
    async execute(command: GetRecommendedBooksCommand): Promise<GetRecommendedBooksDTO> {
        const bookId = new BookId(command.bookId);
        const reviews = await this.reviewRepository.findAllByBookId(bookId);

        const recommendedBooks = this.bookRecommendationService.calculateTopRecommendedBooks(
            reviews,
            command.maxCount
        );
