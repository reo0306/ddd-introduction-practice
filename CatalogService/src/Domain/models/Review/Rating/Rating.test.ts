...
describe('Rating', () => {
    ...
    test("最小値未満の値でRatingを生成するとエラーを投げる", () => {
        expect(() => new Rating(0)).toThrow("評価は1から5の数値でなければなりません.");
    });

    test("最大値超過の値でRatingを生成するとエラーを投げる", () => {
        expect(() => new Rating(6)).toThrow("評価は1から5の数値でなければなりません.");
    });

    test("整数値以外の値でRatingを生成するとエラーを投げる", () => {
